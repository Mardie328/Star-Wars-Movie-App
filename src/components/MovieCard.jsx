...
const MovieCard = () => {
  ...
  useEffect(() => {
    fetch("https://swapi.dev/api/films")
      ...
      .then((actualData) => {
        const updatedData = actualData.results.map((movie) => {
          ...
          return {
            ...movie,
            opening_crawl:
              movie.opening_crawl.length > 300
  ...
  return (
    <>
      ...
      {error && (
        <div>{`This is an HTTP Error: THe status is ${response.status}`}</div>
      )}

      {data &&
        data.map((item) => {
          return (
            <div key={item.episode_id} className="movie-card-container">
              <div>
              ...
              <div className="opening-crawl">{item.opening_crawl}</div>
